...
NS_LOG_COMPONENT_DEFINE ("DsraMain12");
...
void flow_monitor(Ptr<FlowMonitor> monitor, FlowMonitorHelper &flowmon)
{
    ...
    string fileName = "dsra.xml";
    ofstream os(fileName, std::ios::out | std::ios::binary);
    ...
    for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator iter = stats.begin (); iter != stats.end (); ++iter)
    {
        ...
        delayHistogram.SerializeToXmlStream(os, 0, "delayHistogram");
    }
...
void test(int argc, char *argv[])
{
    ...
    LogComponentEnable ("DsraMain12", LOG_LEVEL_INFO);
    ...
    // interface down
    DsraUtils::randomLinkError(nodes, error);
