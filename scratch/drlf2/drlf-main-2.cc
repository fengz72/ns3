...
NS_LOG_COMPONENT_DEFINE ("DrlfMain2");
...
void test(int argc, char *argv[])
{
    ...
    LogComponentEnable ("DrlfMain2", LOG_LEVEL_INFO);
//    LogComponentEnable ("DrlfMain1", LOG_LEVEL_LOGIC);
//    LogComponentEnable ("DrlfUtil", LOG_LEVEL_FUNCTION);
//    LogComponentEnable("DrlfRouting", LOG_LEVEL_FUNCTION);

    NS_LOG_INFO("ttl = " << ttl << ", len = " << len);
...
//    DrlfUtils::LinkSatAndGs(nodes.Get(DrlfConfig::Instance()->GetSatId(satPair2)), gss.Get(1), satPair2);
    ...
    // sat down
    DrlfUtils::squareError(nodes, len);
