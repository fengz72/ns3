...
NS_LOG_COMPONENT_DEFINE ("DraUtils");

void
DraUtils::topo(NodeContainer &nodes, int P, int n, Ipv4RoutingHelper &routingHelper)
{
...
void
DraUtils::mobility_1584(NodeContainer& nodes)
{
...
void
DraUtils::randomLinkError(NodeContainer& nodes, uint32_t limit)
{
...
void
DraUtils::squareError(uint32_t limit)
{
    NS_LOG_FUNCTION(limit);

    uint16_t P = DRAConfLoader::Instance()->getOrbits();
    uint16_t S = DRAConfLoader::Instance()->getSats();
    ...
    for (uint32_t i = 0; i < limit; ++i)
    {
        for (uint32_t j = 0; j < limit; ++j)
        {
            ...
            uint16_t id = DRAConfLoader::Instance()->GetSatId(make_pair<uint16_t, uint16_t>(p, s));
            satBreak(id);
        }
    }
}
...
void
DraUtils::satBreak(uint16_t id)
{
    NS_LOG_FUNCTION(id);
    Ptr<Node> node = DRAConfLoader::Instance()->getNodeContainer().Get(id);
...
void
DraUtils::AssignIp(Ptr<Node> node, pair<uint16_t , uint16_t> nodePair, int type)
{
...
void
DraUtils::LinkSatAndGs(Ptr<Node> sat, Ptr<Node> gs, pair<uint16_t , uint16_t> satPair) {
    ObjectFactory m_queueFactory;   //!< Queue Factory
    ObjectFactory m_deviceFactory;  //!< Device Factory
    ObjectFactory m_channelFactory; // channel factory
    m_queueFactory.SetTypeId("ns3::DropTailQueue<Packet>");
    ...
    Ptr<DRARouting> dra = sat->GetObject<DRARouting>();
    dra->AddGs(ip, sat->GetNDevices() - 1);
}
